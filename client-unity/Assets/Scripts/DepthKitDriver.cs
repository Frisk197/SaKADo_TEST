...
namespace Anaglyph.XRTemplate.DepthKit
{
	...
	{
		public Matrix4x4 dk_InvProj = new Matrix4x4();
		Matrix4x4 dk_InvView = new Matrix4x4();
		...
		public void UpdateCurrentRenderingState(int eye)
		{
			if (!DepthAvailable)
				return;
			...
			var desc = Utils.GetEnvironmentDepthFrameDesc(eye);

			Shader.SetGlobalMatrix(nameof(dk_InvProj), CalculateDepthProjMatrix(desc).inverse);
			Shader.SetGlobalMatrix(nameof(dk_InvView), (CalculateDepthViewMatrix(desc) * trackingSpace.worldToLocalMatrix).inverse);
